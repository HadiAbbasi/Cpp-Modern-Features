...
# اشاره گر shared_ptr چه مشکلی را حل کرد؟
...
بعد از ایجاد `p1`،  معمولاً مقدار `1` است.
...
اکنون مقدار `3` خواهد بود.

متد `use_count()` برای مشاهده و debugging مفید است، اما نباید منطق اصلی برنامه را بر اساس مقدار آن طراحی کنیم.

---

# یک نکته مهم درباره ()use_count

متد `use_count()` یک ابزار مناسب برای پرسیدن این سؤال نیست که «آیا فقط من مالک هستم؟».

بهتر این است که طراحی برنامه را بر اساس قرارداد مالکیت انجام دهیم، نه اینکه دائماً شمارنده را بررسی کنیم.

همچنین در محیط‌های چندنخی، مقدار `()use_count` ممکن است بلافاصله بعد از دریافت آن دیگر همان مقدار قبلی نباشد.

بنابراین بهتر است از `use_count()` بیشتر برای مشاهده، تشخیص و debugging استفاده کنیم، نه برای تصمیم‌های حساس در طراحی.

---

# ساخت shared_ptr

در C++11 می‌توانیم مستقیماً از `new` در نمونه سازی اشیای داخل Heap و اشاره به آنها توسط Shared Pointer استفاده کنیم:
...
نکته مهم این است که اگر از C++11 استفاده می‌کنید، `make_shared` از همان ابتدا در استاندارد وجود داشته است.
...
شی caller می‌تواند یک `shared_ptr` موجود را set کند:
...
اگر تابع فقط می‌خواهد از `shared_ptr` استفاده کند و قرار نیست مالکیت جدیدی ایجاد کند، می‌توان از:
...
اکنون احتمالا باید مقدار `2` را نشان دهد‌.

---

# اگر واقعا بخواهیم خود شی اصلی را Duplicate کنیم تا دو نمونه شی همانند هم به صورت جداگانه داشته باشیم چه کار باید بکنیم؟
...
# متد get() مالکیت را منتقل نمی‌کند
...
پس بعد از دسترسی به شی raw نباید به صورت زیر آنرا delete کنید بلکه باید حذف شی را به عهده اشاره گر هوشمند بسپارید!:
...
// اکنون raw نگهداری شده، اما object دیگر وجود ندارد //
```
...
در ساختار قبلی می‌توانیم یکی از روابط را به شکل زیر تغییر دهیم:
...
# اشاره گر های نوع shared_ptr و weak_ptr چه تفاوتی دارند؟
...
اگر null بودن شی get شونده امکان پذیر است، از اشاره گر raw نوع const به صورت غیر مالک استفاده می شود!
```cpp
const Engine* getEngine() const
{
    return engine.get();
...
در نتیجه این طراحی کاملاً طبیعی است:
...
# اشاره گر shared_ptr و انتقال با move
...
در این حالت reference count افزایش پیدا نمی‌کند؛ ownership state خود `shared_ptr` منتقل می‌شود.
...
اگر `ptr` آخرین مالک بوده باشد، object نیز حذف می‌شود.
...
بعد از `reset()`، شی ساخته شده در Heap هنوز زنده است، زیرا `p2` همچنان مالک آن است.
...
یک `shared_ptr` خالی ایجاد می‌کند.

بررسی null بودن اشاره گر Shared_ptr:
...
# اشاره گر shared_ptr و چند thread
...
اگر دو thread هم‌زمان این کار را انجام دهند:
...
# اشاره گر shared_ptr و destructor
...
# استفاده از اشاره گر shared_ptr چه زمانی انتخاب مناسبی نیست؟
...
اگر چند بخش واقعاً باید مالک object باشند:
...
فقط یک نفر کلید را در اختیار دارد.

اشاره گر `shared_ptr` مانند سیستمی است که چند نفر نسخه‌ای از حق مالکیت یک منبع را دارند.

تا زمانی که حداقل یک نفر هنوز مالک است، منبع باقی می‌ماند.
...
### موارد مناسب برای استفاده از `std::shared_ptr`

اشاره گر `std::shared_ptr` زمانی مناسب است که **مالکیت یک Object واقعاً بین چند بخش مستقل از برنامه مشترک باشد** و بخواهیم Object تا زمانی که آخرین مالک از بین نرفته است، زنده بماند.

برخی نمونه‌های رایج:

* **اشتراک یک Object بین چند Component:** چند Component به یک Object مشترک نیاز دارند و هرکدام در طول عمر خود می‌توانند مالک آن باشند.
* **در Factoryها:** وقتی Factory یک Object را ایجاد می‌کند و می‌خواهد مالکیت آن را به Caller منتقل کند و در عین حال امکان وجود چند مالک را نیز فراهم کند.
* **در Graphها و ساختارهای پیچیده:** در ساختارهایی که چند Object می‌توانند به یک Object دیگر اشاره کنند و مالکیت آن بین چند بخش تقسیم شده است.
* **Cacheها:** وقتی Objectهای موجود در Cache هم‌زمان توسط بخش‌های دیگری از برنامه استفاده می‌شوند و نباید صرفاً با حذف شدن از Cache فوراً Destroy شوند.
* **منابع با چند مصرف‌کننده مستقل:** زمانی که چند بخش مستقل از برنامه به یک Resource نیاز دارند و هیچ‌کدام به‌تنهایی مسئول کامل طول عمر آن نیستند.

**نکته مهم:** صرف اینکه چند بخش از یک Object استفاده می‌کنند، به این معنی نیست که باید از `std::shared_ptr` استفاده کنیم. ابتدا باید مشخص کنیم **چه کسی مالک Object است و چه زمانی باید Destroy شود**. اگر یک مالک مشخص وجود دارد، معمولاً `std::unique_ptr` انتخاب مناسب‌تری است و سایر بخش‌ها می‌توانند صرفاً با Reference یا Pointer غیرمالک به Object دسترسی داشته باشند.
...
استفاده از زیر روش پیشنهادی برای ساخت `shared_ptr` در C++11 است.
