...
نکته مهم این است که `weak` فقط به همان Control Block متصل می‌شود.
...
نکته مهم این است که با Copy کردن `weak_ptr`، Object کپی نمی‌شود.
...
متد `lock()` این است که فقط بررسی نمی‌کند Object زنده است.
...
اتفاق خطرناک این است که بعد از گرفتن Raw Pointer، Lifetime آن را خودمان مدیریت نمی‌کنیم.
...
شی Caller سپس خودش تصمیم می‌گیرد:
...
نکته مهم‌تر این است که همین ویژگی اجازه می‌دهد چرخه‌های Ownership را بشکنیم.
